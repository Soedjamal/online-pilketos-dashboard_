...
const VotersXII = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const { data } = await supabase.from("students_x").select("id, name").eq("voted", true).order("id", { ascending: true })
      setVotersTrue(data);
    };

    const fetchFalseData = async () => {
      const { data } = await supabase.from("students_x").select("id, name").eq("voted", false).order("id", { ascending: true })
      setVotersFalse(data);
    };
    fetchFalseData()
    fetchData();
