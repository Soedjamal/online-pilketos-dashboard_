...
const VotersTS = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const { data } = await supabase.from("teacher_and_staff").select("id, name").eq("voted", true).order("id", { ascending: true })
      setVotersTrue(data);
    };

    const fetchFalseData = async () => {
      const { data } = await supabase.from("teacher_and_staff").select("id, name").eq("voted", false).order("id", { ascending: true })
      setVotersFalse(data);
    };
    fetchFalseData()
    fetchData();
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell>Nama Guru & Staf Karyawan</TableCell>
